...
# This function was manually added
...
function recompute_sum_pair_products(state::IsingState)
    sum = 0
    for i = 1:state.base_length
        for j = 1:state.base_length
...
sum_neighbours(s::IsingState, i, j) =
    sum_neighbours_row(s, i, j) + sum_neighbours_col(s, i, j)
sum_neighbours_row(s::IsingState, i, j) =
    sign(s.matrix[wrap(i - 1, s.base_length), j]) +
    sign(s.matrix[wrap(i + 1, s.base_length), j])
sum_neighbours_col(s::IsingState, i, j) =
    sign(s.matrix[i, wrap(j - 1, s.base_length)]) +
    sign(s.matrix[i, wrap(j + 1, s.base_length)])
wrap(i, L) =
    if i == 0
        L
...
(log_potential::IsingLogPotential)(state::IsingState) =
    log_potential.beta * state.sum_pair_products

# Reference distribution uses beta = 0...
Pigeons.default_reference(log_potential::IsingLogPotential) =
    IsingLogPotential(0.0, log_potential.base_length)
...
Pigeons.initialization(log_potential::IsingLogPotential, ::AbstractRNG, ::Int) =
    IsingState(falses(log_potential.base_length, log_potential.base_length))
...
function Pigeons.step!(explorer::IsingMetropolis, replica, shared)
    ...
    for k = 1:explorer.n_steps
        for i = 1:replica.state.base_length
            for j = 1:replica.state.base_length
